...
## NMDS using all data (corals + symbiodinium)
...
#manhattan distance
library(labdsv)
nsaf.mht<-dist(nsaf3, "manhattan")
mht.pco<-pco(nsaf.mht, k=10)
plot(mht.pco)

dis.can <- dist(nsaf3,"canberra")
can.pco <- pco(dis.can,k=10)
plot(can.pco)


dis.euc <- dist(nsaf3,'euclidean')
disana(dis.euc)
euc.pco <- pco(dis.euc,k=6)
plot(euc.pco)

po<-euc.pco$points[c(2,17,24),]
wn<-euc.pco$points[c(3,18,25),]
wo<-euc.pco$points[c(4,12,19),]
pn<-euc.pco$points[c(1,16,23),]
on<-euc.pco$points[c(6:8,14,15,21,22,27),]
oo<-euc.pco$points[c(5,9,10,11,13,20,26,28),]

plot(euc.pco, type= "n", display="sites")  
points(pn, pch=16, col=colors[2],cex=1.6)
points(po, pch=16, col=colors[1],cex=1.3)
points(wn, pch=17, col=colors[2],cex=1.6)
points(wo, pch=17, col=colors[1],cex=1.3)
